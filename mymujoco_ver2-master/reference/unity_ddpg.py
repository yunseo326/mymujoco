...
step_print_interval = 100  # 100번의 스텝마다 진행 상황 출력  ###### 1개의 episode가 너무나도 길어서 확인불가.... 그래서 추가한 코드
...
if os_name == 'Windows':
    env_name = "C:/5 colledge file/2024-1/hands/ml-agents-release_17/Project2/Env/robotarm28/Robotarm.exe"  # 성공한거 5
elif os_name == 'Darwin':
    env_name = f"../envs/{game}_{os_name}"

# 모델 저장 및 불러오기 경로
date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
save_path = f"C:/5 colledge file/2024-1/hands/ml-agents-release_17/Project2/saved_models/Robotarm/DDPG4/{date_time}"
load_path = "C:/5 colledge file/2024-1/hands/ml-agents-release_17/Project2/saved_models/Robotarm/DDPG4/20240926215419"
...
class DDPGAgent():
    ...
    def get_action(self, state, training=True):
        ...
        action = np.clip(action + self.OU.sample(), -1, 1)  # 액션 값을 -1, 1 사이로 클리핑
...
if __name__ == '__main__':
    ...
    for step in range(run_step + test_step):
        ...
        episode_step_count += 1 ## 마찬가지 1개의 에피소드가 너무나도 길어서 확인하고자 하는 코드임 action 횟수
        if episode_step_count % step_print_interval == 0:
            print(f"Step {step}: Episode {episode}, Current Score: {score:.2f}, Step Count: {episode_step_count}") #### 여기까지
            print(f"Action: {action}")


        if train_mode:
            #agent.append_sample(state[0], action[0], reward, next_state[0], [done])
            agent.append_sample(dec.obs[0], action, reward, next_state, [done])
        ...
        if train_mode and step > max(batch_size, train_start_step):
            ...
            for i in range(action_size):
                if action[0][i] is not None:
                    agent.writer.add_scalar(f"action/value_{i}", action[0][i].item(), step)
                ...
                if actor_loss is not None and critic_loss is not None:
                    ...
                    agent.write_summray(score, actor_loss, critic_loss, step)
