...
class QNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.fc1 = nn.Linear(np.array(env.single_observation_space.shape).prod() + env.action_size, 64)
...
if __name__ == "__main__":
    ...
    register(id='ball_example', entry_point='model.moving_ball:Boxenv',)
    ...
    envs = gym.make('ball_example')  # MuJoCo 기반 환경
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:  # 탐험을 위해 actor보다는 random에서 값을 가져와서
            ...
            with torch.no_grad():  
                ...
                # print([(envs.action_high-envs.action_low)//2,(envs.action_high-envs.action_low)//2],args.exploration_noise)
